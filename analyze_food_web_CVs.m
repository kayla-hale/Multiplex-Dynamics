% This script analyzes solutions of Rewards Only Food Web (FW) simulations.
% Specifically, it calculates BIOMASS VARIABILITY for the final community,
% each guild, and species at the end of simulations. 
% It generates a single file: Results_RO_FW_CVs.txt with column headings 
...
for i = 1:numel(samples)
    ...
    % begin analysis
    % species persistence
    surviving_species = solution(1:this_S,final_timestep) > extinction_threshold;
    persistence(i,1) = sum(surviving_species)/this_S;

    solution = solution(:,final_timestep-1000:final_timestep);
...
results = [simID, time_step, beta, S, persistence, avg_species_CV, avg_guild_CV_stzd, community_CV, guild_results];

% remove simulations that weren't run
results(not_run,:) = [];

% set up labels
VariableNames = {'simID','time_step','beta','S','persistence','avg_species_CV','avg_guild_CV_stzd','community_CV',...
...
summary_table = array2table(results,'VariableNames',VariableNames);

% save simulation summary table
writetable(summary_table,'Summary_RO_FW_CVs.txt')
